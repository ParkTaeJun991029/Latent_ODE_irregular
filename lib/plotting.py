...
def plot_trajectories(ax, traj, time_steps, min_y = None, max_y = None, title = "", 
	...
	for i in range(traj.size()[0]):
		d = traj[i].cpu().numpy()[:, dim_to_show] # 수정
...
class Visualizations():
	...
	def init_visualization(self):
		...
		self.ax_samples_same_traj = self.fig.add_subplot(2,3,5, frameon=False)
	...
	def draw_all_plots_one_dim(self, data_dict, model,
		...
		# 	reconstructions[:,:,dim_to_show].detach().cpu().numpy().max()) # detach() 추가
		# min_y = min(
		# 	data_for_plotting[:,:,dim_to_show].cpu().numpy().min(),
		# 	reconstructions[:,:,dim_to_show].detach().cpu().numpy().min()) # deatch() 추가

		############################################
		# Plot reconstructions, true postrior and approximate posterior
		n_traj_to_show_1 = data_for_plotting.shape[0] 
		cmap = plt.cm.get_cmap('Set1')
		for traj_id in range(3): #range(3)을 수정 range(num_trajectories)
			# Plot observations
			...
			plot_trajectories(self.ax_traj[traj_id],
				...
				min_y = min_y, max_y = max_y, title="Sample {} lc_cureve of {} (data space)".format(traj_id,n_traj_to_show_1), dim_to_show = dim_to_show,
		...
		samples_same_traj, _ = model.get_reconstruction(time_steps_to_predict, 
			observed_data[:1], observed_time_steps, mask = observed_mask[:1], n_traj_samples = 1)
		samples_same_traj = samples_same_traj.squeeze(1)
		
		plot_trajectories(self.ax_samples_same_traj, samples_same_traj, time_steps_to_predict, marker = '',max_y = 2,min_y = -1)
		plot_trajectories(self.ax_samples_same_traj, one_traj, time_steps, linestyle = "", max_y = 2,min_y = -1,color = cmap(2),
		...
		if isinstance(model, LatentODE):
			...
			traj_from_prior = model.sample_traj_from_prior(time_steps_to_predict, n_traj_samples = 10)
			#print("shape",traj_from_prior.shape)
			...
			plot_trajectories(self.ax_traj_from_prior, traj_from_prior, time_steps_to_predict, 
				marker = '', linewidth = 3,max_y = 2,min_y = -1)
